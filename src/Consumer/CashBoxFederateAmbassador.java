...
public class CashBoxFederateAmbassador extends NullFederateAmbassador
{
	    ...
	    throws FederateInternalError
	{
		...
		if( interactionClass.equals(federate.getClientToQueueHandle) )
		{
//			builder.append( " (DrinkServed)" );
			byte[] bytes = theParameters.get(federate.productsNumberHandle);
			HLAinteger32BE productsNumber = new HLA1516eInteger32BE();
			try {
				productsNumber.decode(bytes);
			} catch (DecoderException e) {
				e.printStackTrace();
			}
			int productsNumberValue = productsNumber.getValue();
		}
